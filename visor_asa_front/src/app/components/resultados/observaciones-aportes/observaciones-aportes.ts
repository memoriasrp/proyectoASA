...
@Component({
  ...
  templateUrl: './observaciones-aportes.html',
  styleUrl: './observaciones-aportes.css',
})
export class ObservacionesAportes implements OnInit {
  aportes: any[] = [];
  searchTerm: string = '';
  ...
  cargarTabla() {
    this.observacionesAportesService.getAportesPaginados(this.currentPage, 20, this.searchTerm, this.monedaSeleccionada)
      .subscribe({
        next: (res) => {
  ...
  exportarExcel(): void {
    this.observacionesAportesService.getAportesParaExportar(this.searchTerm, this.monedaSeleccionada)
      .subscribe({
        next: (res) => {
