...
import { ObservacionesAhorrosService } from '../../../services/resultados/observaciones-ahorros-service';
import { SeguimientoHistorialService } from '../../../services/vbcoop/seguimiento-historial-service';

@Component({
  selector: 'app-observaciones-ahorro',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './observaciones-ahorro.html',
  styleUrl: './observaciones-ahorro.css',
})
export class ObservacionesAhorro implements OnInit {
  ahorros: any[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private observacionesAhorrosService: ObservacionesAhorrosService,
  ...
  cargarTabla() {
    this.observacionesAhorrosService.getAhorrosPaginados(this.currentPage, 20, this.searchTerm, this.monedaSeleccionada)
      .subscribe({
        next: (res) => {
  ...
  exportarExcel(): void {
    this.observacionesAhorrosService.getAhorrosParaExportar(this.searchTerm, this.monedaSeleccionada)
      .subscribe({
        next: (res) => {
  ...
  guardarSeguimiento() {
    ...
    formData.append('tipoproducto', 'AHORRO');
